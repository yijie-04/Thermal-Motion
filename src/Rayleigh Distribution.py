...
plt.figure(3)
plt.subplot(2, 1, 1)
plt.hist(d, bins=num, density=True, color = "#336ea0", ec="black", lw=1)
plt.title('Histogram Fitted with Rayleigh Distribution')
...
plt.plot(bin, rayleigh(bin, *popt), '-', label="best fit", color="red", lw=1.5)
plt.legend()
...
residuals = hist - rayleigh(bin, 0.1667)
print(residuals)
plt.subplot(2, 1, 2)
plt.scatter(bin, residuals, color='#1f77b4')
plt.axhline(0, color='red', linestyle='--')
plt.title('Residuals of Rayleigh Distribution Best Fit Curve')
plt.xlabel('Distance [μm]')
plt.ylabel('Probablity')
plt.tight_layout()
